...
# Let's send output to a log and see if at the terminal
exec > >(tee /var/tmp/annotations-test.log) 2>&1
...
createNs() {
  ...
  while [[ `sudo oc get ns | grep $POD_NS | wc -l` -gt 0 ]] ; do
    echo "[createNs]: Waiing on Frenzy to destroy NS: $POD_NS `oc get ns | grep $POD_NS`"
    sleep 3
  done
  echo "Done waiting!"
  oc create -f ./perceptorTestNS.yml
  while [[ `sudo oc get ns | grep $POD_NS | wc -l` -lt 0 ]] ; do
    echo "[createNs]: Waiing on NS to come up: `oc get ns | grep $POD_NS`"
    sleep 3
  done
}
...
createPod() {
  ...
  until [[ `oc get pods | grep -v STATUS | wc -l` -ge "$PODS" ]] ; do
    echo "[createPod]: Waiting on PODs to come up, so far: `oc get pods | grep -v NAME`"
    sleep 3
  done
  echo "Done waiting!"
  for i in `oc get pods | grep -v build | grep -v deplopy | grep -v NAME | cut -d ' ' -f 1` ; do
    tstAnnotate $i
    retVal=0
    passed=0
    failed=0
      if [[ retVal -gt 0 ]] ; then
        ...
        echo "[createPod] Passed POD Annotations test on $i!"
        (( passed++ ))
      fi
  done
